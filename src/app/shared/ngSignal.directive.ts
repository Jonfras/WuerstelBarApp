...
console.log('using NgSignalDirective v2.1.4 [2024-02-22]');
...
export class NgSignalDirective<T> implements OnInit {
  ...
  private setValueToSignal(): void {
    ...
    if (this.valueType === 'boolean' || this.valueType === 'object') {
      try { value = JSON.parse(value); }
      catch (exc) { console.error(`Cannot parse as JSON: ${value} - missing json pipe (i.e. [value]="item|json")?`); }
    }
